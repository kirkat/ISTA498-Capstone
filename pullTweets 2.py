    ...
    try:
        ...
        for opt, arg in opts:
            ...
            #     username_files.add(arg)
                ...
                start = arg.split("-")
                for i in range(3):
                    start[i] = start[i].zfill(2)
                start = np.datetime64("-".join(start))
                end = start + np.timedelta64(6*7, 'D')
                weekStart = np.arange(start,end,7,dtype='datetime64[D]')
                weekEnd = np.empty(weekStart.size, dtype = 'datetime64[D]')#.reshape(weekStart.size,1)
                for i in range(weekStart.size):
                    weekEnd[i] = weekStart[i] +  np.timedelta64(7, 'D')
                argMap["since"] = weekStart
                argMap["until"] = weekEnd
        ...
        def receiveBuffer(tweets):
            ...
            if sys.stdout.isatty():
                print("\rSaving tweets", end='', flush=True)
            else:
                print(cnt, end=' ', flush=True)

        print("Downloading tweets for "+argMap["query"]+" from "+ str(argMap["since"][0]) +" to "+str(argMap["until"][-1]))

        for i in range(len(argMap["since"])):
            
            
            tweetCriteria = got.manager.TweetCriteria()
            newFolder = argMap["query"]
            if not os.path.exists(newFolder):
                os.makedirs(newFolder)
            outputFileName = newFolder +"/"+"week"+str(i)+".csv"   #" from "+str(argMap["since"][i]) +" to "+str(argMap["until"][i])+".csv"
            outputFile = open(outputFileName, "w+", encoding="utf8")
            tweetCriteria.querySearch = argMap["query"]
            tweetCriteria.since = str(argMap["since"][i]) 
            tweetCriteria.until = str(argMap["until"][i])
            if "max" in argMap:
                tweetCriteria.maxTweets = int(argMap["max"])
            ...
            print(" from "+ str(argMap["since"][i]) +" to "+str(argMap["until"][i]))
        print("\rSaved %i"%all, end='', flush=True)
        ...
        if "outputFile" in locals():
            outputFile.close()
            print()
            print('Done. Output file generated "%s".\n\n' % newFolder)
